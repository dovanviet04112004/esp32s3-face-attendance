...
| LCC-FASD | Bản Kaggle [`faber24/lcc-fasd`](https://www.kaggle.com/datasets/faber24/lcc-fasd), script `00_fetch_raw.sh` tải bằng token Kaggle khai trong `ml/.env`, không form; [bài báo gốc](https://csit.am/2019/proceedings/PRIP/PRIP3.pdf) (ID R&D 2019) không vào được — **chưa tải**, manifest đã ghi | 5,19 GB PNG, bố cục `LCC_FASD/LCC_FASD_{training,development,evaluation}/{real,spoof}/` | test khác miền — thật, in, phát lại **chụp lại bằng điện thoại trong phòng thường**, thật và giả cùng cảnh. Xét trộn vào train **sau** khi chấm chéo |
| SynthASpoof | Google Drive **công khai** (`SynthASpoof.zip`, id ghi trong manifest, script `00_fetch_raw.sh` tải bằng gdown), [GitHub](https://github.com/meilfang/synthaspoof) — **chưa tải** | zip 12 GB: 25.000 thật tổng hợp + 78.800 tấn công | test khác miền — in và phát lại qua 1 điện thoại, 2 tablet, 1 webcam. CC BY-NC-SA 4.0, chỉ nghiên cứu, không sản phẩm |
