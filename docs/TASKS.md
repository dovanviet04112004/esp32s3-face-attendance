...
| **E14-T8** | **Vẽ sơ đồ nguyên lý + PCB bằng KiCad 10.0.6** — chép chân từ §2, không chép ngược. **Sơ đồ XONG 12/09**: 15 linh kiện, 22 net (loa về domino của chính module MAX98357A nên board đế không mang đầu nối loa, §2.3E), **ERC 0**. Mười bảy cảnh báo trước đó không phải nhiễu: `kiosk.kicad_sym` và footprint tự vẽ đều bị đóng dấu sai phiên bản định dạng nên KiCad không mở nổi thư viện; sửa đúng số là hết sạch. **Xếp linh kiện XONG 12/09**: board **158 × 121** (chiều cao do tấm màn 107 mm cộng chỗ in tên chân hai đầu quyết định), 24 linh kiện kể cả 4 lỗ vít màn, 4 lỗ bắt board vào vỏ và hàng lỗ microSD, 15/15 chân devkit khớp `app_config.h` qua `tools/check_schematic.py` và board khớp sơ đồ qua `tools/check_pcb.py`, **DRC 0**. Tám cảnh báo "dấu chân không khớp thư viện" trước đó cũng không phải nhiễu: KiCad ghi góc xoay của module vào **từng pad** (`(at 0 0 180)`), phép so dấu chân đọc góc đó, mà generator chép pad nguyên xi từ thư viện nên pad nào cũng mang góc 0. Chứng minh bằng board demo `ecc83` của chính KiCad: xoay 6 module ở đó mà chỉ sửa đúng dòng `(at)` thì ra đúng 6 cảnh báo y hệt. Và không còn một chữ in lụa nào nằm dưới thân module — không tên chân, không tên linh kiện. Nguồn sinh ra toàn bộ file KiCad nằm ở `hardware/gen/`, chạy lại ra file y hệt. `kicad-cli.exe` ở `/mnt/d/KiCad/bin/` chạy được từ WSL nên ERC, DRC và xuất file không cần thao tác tay. Còn lại: **đi dây** (chờ duyệt bố cục), và ba khung module còn là ước lượng 🔬 (LCD, VL53L1X, MAX98357A) | Mọi chân khớp §2 và `app_config.h`; PDF mở ra đọc được ở A4 | E1-T9 |
