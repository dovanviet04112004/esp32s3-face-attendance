...
| **E14-T8** | **Vẽ sơ đồ nguyên lý + PCB bằng KiCad** — chép chân từ §2, không chép ngược. **Sơ đồ XONG 12/09**: 16 linh kiện, 29 net, 16/16 footprint đối chiếu với thư viện KiCad thật, ERC từ 150 còn 17 (đều là "thư viện `kiosk` chưa đăng ký", `kicad-cli` không đọc bảng cấp project — xác nhận lại bằng GUI). PDF ở `hardware/export/kiosk_schematic_4ef6f05.pdf`, A3 một trang. `kicad-cli.exe` ở `/mnt/d/KiCad/bin/` chạy được từ WSL, nên ERC và xuất PDF không cần thao tác tay. Còn lại: **PCB** (đường bao footprint chờ đo dài × rộng devkit — lỗ khoan thì đã đủ) | Mọi chân khớp §2 và `app_config.h`; PDF mở ra đọc được ở A4 | E1-T9 |
