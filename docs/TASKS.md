...
| **E9-T11** | **Train lại anti-spoof trên kiến trúc mới** — `ReLU`, `AvgPool2d` cỡ cố định, đầu vào 81. Run `20260909-1116` mới tới epoch 2/60. Nhánh này **đã rút khỏi lock**, ảnh `models_0` chỉ còn 2 nhánh cho tới khi có bản train xong. **Đề xuất 11/09, chờ duyệt**: 470 ms hiện tại toàn conv thật, nút còn lại là thu **nhánh wide** (đọc ngữ cảnh thô, không cần chi tiết) — bảng đối chứng §4.2 với 3 arm cùng seed/split/epoch: (A) tight 32 + wide 32 như hiện tại, (B) tight 32 + wide 16, (C) tight 32 + wide 16 @ 49 px; so ACER trên split `test` và latency ở `bench_ai`. `MiniFASNetV2SE` hiện dùng **một** `width` cho cả hai backbone (`minifasnet_v2_se.py:98-99`), arm B/C cần thêm `wide_width` (và `wide_input`) — nửa sau của E9-T12. Ngưỡng live chốt theo mục tiêu tỉ lệ từ chối người thật trên split `test`, và pipeline chỉ chấm spoof khi mặt gần chính diện (§4.5.5d) | ACER < 5% ở INT8, chấm lại trên tập tự thu E3-T8, export, vào lock | E9-T3, E3-T8 |
