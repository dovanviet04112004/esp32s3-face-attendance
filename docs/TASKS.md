...
| E7-T6 | `drv_touch` (GT911, trình tự chọn địa chỉ) — **init chạy thật 09/09**: `gt911 at 0x5D, 320x480`, product ID `0x39 0x31 0x31`. Còn nợ: chạy lại bộ `test_apps/touch` để chốt toạ độ sau khi siết dây | Đọc được điểm chạm | E7-T4 |
...
| **E7-T16** | **Khấc ngang khi ảnh trong preview di chuyển.** Panel quét lại GRAM của nó mỗi ~17 ms 🔬 trong khi một khung 307 KB mất **32,3 ms** để ghi (đo, sau khi nâng SPI 40 → 80 MHz; ở 40 MHz là **59,8 ms**), nên trên kính luôn có phần khung mới cạnh phần khung cũ. Nâng 80 MHz giảm một nửa, không hết. Hạ tốc độ quét panel bằng `FRMCTR1 0xB1` **làm nặng thêm** (thử `DIVA=/8`: khấc rõ hẳn) vì dòng cũ đứng trên kính lâu hơn — chứng minh cơ chế đúng là tia quét, và chứng minh khoá pha là điều kiện bắt buộc chứ không phải chỉ hạ tốc độ. Không có chân TE trên breakout, nhưng **`SDO` đọc được thật**: nối `SDO` → `GPIO43`, bus 2 MHz, ghi `MADCTL` rồi đọc lại thì số đọc đi theo số ghi (`0x00`→`0x00`, `0xC0`→`0xC0`). Chưa dùng được ngay: cùng đầu vào `0x48` ra hai giá trị khác nhau giữa hai lần, và hạ bus xuống 500 kHz thì mọi thanh ghi ra `0xFF` — biên độ thời gian chưa chốt. `0x0C` và `0x04` không phục vụ, chỉ `0x0B` trả dữ liệu | Chốt được biên độ đọc rồi đọc `GET_SCANLINE (0x45)` ra số dòng tăng dần và cuộn vòng → khoá pha, ghi đuổi ngay sau tia quét, khấc về 0. Đọc không ổn định thì giảm số byte mỗi khung (320×240 = 16 ms 🔬) và ghi giới hạn vật lý vào §2.3A | E7-T11 |
