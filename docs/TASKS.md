...
| E7-T5 | `drv_lcd` (ST7796 + bounce buffer) + LVGL port — **SPI chạy, đèn nền KHÔNG**, xem E7-T15 | Hiện được ảnh tĩnh 320×480 | E7-T2 |
| **E7-T15** | **Đèn nền LCD không sáng, phần mềm đã loại trừ.** Lái GPIO21 bằng GPIO thô, đảo mức 5 s/lần, màn tô trắng kín — **tối ở cả hai mức**. Nên không phải LEDC, không phải cực tính, không phải dữ liệu (SPI ghi thành công, `st7796 up at 320x480`). §2.3A đã cảnh báo sẵn: đèn nền > 40 mA thì phải qua MOSFET N, mà chân GPIO ESP32-S3 chỉ đẩy ~20 mA an toàn | Cắm `BLK` thẳng vào 3V3: **sáng** → lắp AO3400 theo §2.3A; **vẫn tối** → đo `VCC` module, nếu cần 5V thì sửa §2.3A theo §1.3 | E7-T2 |
