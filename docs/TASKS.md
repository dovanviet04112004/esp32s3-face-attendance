...
| E8-T9 | 🔬 **Đo RAM đỉnh toàn hệ** — `heap_caps_get_minimum_free_size` cả internal và PSRAM. **App `firmware/test_apps/bench_mem` viết và biên dịch 12/09**, dựng đúng chuỗi `app_boot` + `app_tasks` như `soak` nên số đo là của kiosk thật, in đáy internal/PSRAM lúc trước boot, sau boot và mỗi 10 s khi đang chạy, cộng watermark từng task. **Chưa chạy được: board đang cắm một mình**, `bsp_board_init()` abort vì I2C 0x20 và 0x29 im. Số lấy được ở lượt abort: đáy internal trước boot **243.895 B**, PSRAM 8.189 KB; sau khi nạp 3 model còn **233 KB** internal. Chạy lại khi cắm đủ ngoại vi. §6.4 vẫn nợ 267 KB cho Wi-Fi, LVGL và buffer crop nên chưa đối chiếu được | Số vào `docs/measurements/`, đối chiếu §6.4 | E8-T7, ngoại vi cắm đủ |
...
| **E10-T11** | **P1 của `docs/FREERTOS.md`: `m_facedb` bị giữ suốt phép ghi flash 1,8–2,3 s trong khi `kLockMs` là 200 ms.** `lookup` trong cửa sổ đó hết giờ, và `pipeline.cpp:143` biến mọi lỗi tra cứu thành `UNKNOWN` — người thật bị từ chối vì có ai đó vừa đăng ký xong. Thứ tự khoá đúng, đây là lỗi độ dài vùng khoá. Ba đường: chép ảnh bảng rồi thả khoá mới ghi; chặn enroll chồng xác thực ở `svc_attendance`; hoặc tách lỗi tra cứu khỏi "không tìm thấy". **Phải chốt trước E10-T7** | Enroll chạy song song xác thực không làm rớt người thật | E8-T11 |
| **E10-T12** | **P2 và P3 của `docs/FREERTOS.md`.** P2: `ai_task` đăng ký watchdog rồi chặn `portMAX_DELAY` trên `q_frame`, camera chết thì watchdog nổ và **chỉ sai thủ phạm**. P3: `xQueueSend(results, 0)` không kiểm giá trị trả về, `attend_task` chậm quá 4 kết quả thì một `MATCH` rơi mất không dấu vết | Watchdog chỉ đúng task hỏng; không kết quả nào rơi mà không có log | E10-T4 |
