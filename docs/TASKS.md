...
| **E9-T26** | **`svc_vision` chỉ có cổng đầu xa (`face_min_px` 113), chưa có tiền kiểm hình học của KẾ HOẠCH §3 "Chốt 1"**: hộp mặt phải nằm trọn trong vùng ảnh thật của khung, có lề, không đạt thì trả "đưa mặt vào khung" thay cho phán quyết sống/giả; đầu gần 0,25 m hiện chỉ trông vào ToF. Trên 73 khung board 11/09, 5/51 mặt có hộp nằm ngoài dải 113–191 px và vài khung mặt cắt mép vẫn được chấm | Thêm `SVC_VISION_FACE_OUT_OF_FRAME` (hoặc tên tương đương trong `app_events.h`), kiểm lề trước `verify()`, test app pipeline có ca mặt chạm mép | E7-T4 |
