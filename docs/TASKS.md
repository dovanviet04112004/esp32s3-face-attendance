...
| **E7-T14** | **`esp_lcd_touch_gt911_read_data` trả `ESP_ERR_INVALID_RESPONSE`** dù `init` vừa đọc config xong ở cùng millisecond. Đã loại: chờ thêm 300 ms sau init (vẫn trượt), mốc `INT_HOLD` 50 và 100 ms (như nhau), khoá bus (`drv_touch_read` có lấy `bsp_i2c_lock`) | Đọc được toạ độ điểm chạm thật; nguyên nhân có bằng chứng, không thêm workaround khi chưa biết vì sao | E7-T6 |
