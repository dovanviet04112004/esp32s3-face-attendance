...
| **E14-T8** | **Vẽ sơ đồ nguyên lý + PCB bằng KiCad 10.0.6** — chép chân từ §2, không chép ngược. **Sơ đồ XONG 12/09**: 15 linh kiện, 22 net (loa về domino của chính module MAX98357A nên board đế không mang đầu nối loa, §2.3E), ERC 16 và đều là "thư viện `kiosk` chưa đăng ký" — `kicad-cli` không đọc bảng cấp project, xác nhận lại bằng GUI. **Xếp linh kiện XONG 12/09**: board **158 × 121** (chiều cao do tấm màn 107 mm cộng chỗ in tên chân hai đầu quyết định), 20 linh kiện kể cả 4 lỗ vít màn và hàng lỗ microSD, 15/15 chân devkit khớp `app_config.h` qua `tools/check_schematic.py` và board khớp sơ đồ qua `tools/check_pcb.py`, DRC 9 và không còn một chữ in lụa nào nằm dưới thân module — không tên chân, không tên linh kiện. Nguồn sinh ra toàn bộ file KiCad nằm ở `hardware/gen/`, chạy lại ra file y hệt. `kicad-cli.exe` ở `/mnt/d/KiCad/bin/` chạy được từ WSL nên ERC, DRC và xuất file không cần thao tác tay. Còn lại: **đi dây** (chờ duyệt bố cục), và ba khung module còn là ước lượng 🔬 (LCD, VL53L1X, MAX98357A) | Mọi chân khớp §2 và `app_config.h`; PDF mở ra đọc được ở A4 | E1-T9 |
