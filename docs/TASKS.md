...
| **E3-T11** | **`core/trainer.py`: thêm `torch.compile`, bật bằng khoá `train.compile` trong config** (không hardcode — nó phải nằm trong `config.resolved.yaml` thì bảng §3.7 mới đối chứng được). Đo được trên teacher: 1 luồng Python ghim 100% một nhân trong khi GPU chỉ 66–71% — nghẽn ở chi phí phóng lệnh, không ở đĩa cũng không ở GPU. Student nhỏ hơn teacher nên tỉ lệ chi phí này còn cao hơn. Sửa một chỗ, **cả 6 run student của 3 nhánh** cùng hưởng. Tham số `compile` của Ultralytics **không dùng được**: teacher detection là run Ultralytics duy nhất và nó đã chạy rồi | Đo it/s trước/sau trên cùng cấu hình; **cả hai arm của một bảng §3.7 phải cùng giá trị** vì compile đổi kernel nên đổi số học ở chữ số cuối; OOM trên card 4 GB thì hạ từ `reduce-overhead` xuống `default` và ghi lại | E2-T5 |
