...
esp_err_t drv_camera_init(void)
{
    ...
    s_ready = true;
    ESP_LOGI(TAG, "sensor 0x%04X up at qvga rgb565 in psram", sensor->id.PID);
    return ESP_OK;
}
...
esp_err_t drv_camera_selftest(void)
{
    ...
    const int fps = elapsed_us > 0 ? (int)((int64_t)taken * 1000000 / elapsed_us) : 0;
    ESP_LOGI(TAG, "%d frame(s) of %ux%u, %u bytes each, %d fps", taken, (unsigned)width,
             (unsigned)height, (unsigned)bytes, fps);
