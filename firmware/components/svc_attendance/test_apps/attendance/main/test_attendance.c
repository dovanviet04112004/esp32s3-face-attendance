...
TEST_CASE("a face that dips under the size gate is still the same arrival", "[svc_attendance]")
{
    machine_up(true);
    back_to_idle();
    svc_attendance_on_presence(true);
    ...
    feed(SVC_VISION_FACE_SMALL, 0, NO_SPOOF_SCORE);
    feed(SVC_VISION_MATCH, EMPLOYEE_A, LIVE_SCORE);
    printf("a step back and a match again left state %d\n", (int)svc_attendance_state());
    TEST_ASSERT_NOT_EQUAL(SVC_ATTENDANCE_GRANTED, svc_attendance_state());
    TEST_ASSERT_FALSE(svc_door_is_open(svc_door_fake()));
}
