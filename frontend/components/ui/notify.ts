 ...
 *  @param fell what the mutation threw, its api code turned into the sentence via lib/fault; or the sentence itself.
 */
export function useNotify(): { done: (title: string, description?: string) => void; failed: (fell: unknown) => void } {
  ...
  return {
    ...
    failed: (fell) => void toasts.add({ title: typeof fell === "string" ? fell : faultOf(fell), variant: "error" }),
  };
}
