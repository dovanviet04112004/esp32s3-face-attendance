#!/usr/bin/env bash
#
# Train the recognition branch. Restarts itself from its own last.pth when the
# run dies, with a ceiling on attempts so a real fault cannot loop forever.
#
# Usage:
#   ./scripts/22_train_recog.sh
#   ./scripts/22_train_recog.sh configs/recognition/mobilefacenet.yaml train.epochs=50
#   ./scripts/22_train_recog.sh configs/recognition/mobilefacenet.yaml \
#       train.resume=artifacts/recognition/runs/<run>/ckpt/last.pth
#
...
MODULE="facepipe.tasks.recognition.train"
RUNS="${ML_ROOT}/artifacts/recognition/runs"
CFG="${1:-configs/recognition/mobilefacenet.yaml}"
[[ $# -gt 0 ]] && shift
OVERRIDES=("$@")
